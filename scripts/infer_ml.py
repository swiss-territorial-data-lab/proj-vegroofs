...
def infer_ml(roofs: gpd.GeoDataFrame, CLS_ML: str, MODEL_ML: str,
             WORKING_DIR: str, PICKLE_DIR: str, STAT_DIR: str = None, ):
    ...
    roofs = roofs.merge(desc_tmp[desc_col_egid],
                        on='EGID', suffixes=('', '_lum'))
    ...
    roofs_pred_pd = pd.concat([pd.DataFrame(roofs[['EGID']]).reset_index(),
                               pd.DataFrame(roofs_pred)], axis=1, ignore_index=True, sort=False
                               ).rename(columns={1: 'EGID', 2: 'pred'})
    if CLS_ML == 'binary':
        roofs_pred_pd = pd.concat([roofs_pred_pd[['EGID', 'pred']],
                                   pd.DataFrame(roofs_proba)], axis=1, ignore_index=True, sort=False
                                   ).rename(
                                       columns={0: 'EGID',
                                                1: 'pred',
                                                2: 'proba_bare',
